...
| 成员 | 纵向开发模块 | 横向文档 |
|---|---|---|
| 组长 | 架构基座、统一 API 规范、认证双体系、RBAC、审计与安全 | 需求文档、进度计划表、工作量占比表、答辩 PPT |
| 组员 A | 前台官网（响应式/首页/PLP/PDP/对比）、B2C 用户中心 | 交互说明、团队例会纪要 |
| 组员 B | B2B 经销商闭环（申请审批/门户/Quick Order/RFQ/PO） | B2B 流程与状态机规范、个人思政报告 |
| 组员 C | 商品与变体 SKU、库存扣减、价格引擎、购物车结算、订单状态机 | 数据字典与 API 规格书、答辩 PPT 技术章节 |
| 组员 D | Admin UI、CMS 与富文本、媒体中心、联系工单、SEO/审计查看 | 系统操作手册、个人思政报告 |
| 组员 E | Docker 编排、Seed 数据、接口自动化与压测、越权/兼容测试 | 全套测试报告、个人思政报告 |
...
| `docs/templates/meeting-minutes.md` | 例会纪要统一模板 |
