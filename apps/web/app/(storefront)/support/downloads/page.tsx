...
export default async function Page({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-14">
      <h1 className="text-4xl font-bold">{t("Downloads")}</h1>
      <p className="my-5">
        {t("Published instructions, certificates and resources.")}{" "}
      ...
      <form className="mb-6 flex flex-wrap items-end gap-3">
        ...
        <button className="rounded border px-4 py-2">{t("Filter")}</button>
        <Link href="/support/downloads" className="p-2 underline">
          {t("Clear filters")}
        </Link>
      </form>
      <p role="status" className="mb-4">
        {t("{count} resources", { count: rows.length })}
      </p>
          ...
          <li key={row.id}>
            ...
            >
              ...
              <p className="mt-2 text-sm">
                {t(row.resourceType)} · {t(row.language)} ·{" "}
                {(row.sizeBytes / 1024).toFixed(1)}KB ·{" "}
                {t("Version {version}", { version: row.version })}
              </p>
              <p className="mt-2 text-sm">
                {t("Published {date}", { date: row.publishedAt.slice(0, 10) })}
              </p>
              {row.tags.length > 0 && (
      ...
      {!result.ok ? (
        <p role="alert">
          {t("Resources are temporarily unavailable. Please try again.")}
        </p>
      ) : (
        !rows.length && <p>{t("No published files match these filters.")}</p>
      )}
    </div>
  );
}
