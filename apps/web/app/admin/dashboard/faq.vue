<template>
  <main class="faq">
    <div class="header">
      <h1>FAQ</h1>
      <button @click="dialogVisible = true">+ New FAQ</button>
    </div>

    <table class="data-table">
      <thead>
        <tr>
          <th>ID</th>
          <th>Question</th>
          <th>Answer</th>
          <th>Category</th>
          <th>Actions</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="f in faqs" :key="f.id">
          <td>{{ f.id }}</td>
          <td>{{ f.question }}</td>
          <td>{{ f.answer }}</td>
          <td>{{ f.category }}</td>
          <td>
            <button @click="save">Edit</button>
            <button class="danger" @click="remove(f.id)">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
  </main>
</template>


<script setup>
import { ref, onMounted } from 'vue'

const API = '/api/v1'
const faqs = ref([])
const dialogVisible = ref(false)
const editing = ref(false)
const form = ref({ id: null, question: '', answer: '', sort: 0 })

const openCreate = () => {
  editing.value = false
  form.value = { id: null, question: '', answer: '', sort: 0 }
  dialogVisible.value = true
}

const edit = (item) => {
  editing.value = true
  form.value = { ...item }
  dialogVisible.value = true
}

const loadFaqs = async () => {
  try {
    const res = await fetch(`${API}/faqs/`)
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    faqs.value = await res.json()
  } catch (e) {
    console.error('加载FAQ失败', e)
  }
}

const save = async () => {
  try {
    const params = new URLSearchParams({
      question: form.value.question,
      answer: form.value.answer,
      sort: String(form.value.sort ?? 0),
    })

    const url = editing.value ? `${API}/faqs/${form.value.id}` : `${API}/faqs/`
    const method = editing.value ? 'PUT' : 'POST'

    const res = await fetch(url, {
      method,
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: params.toString(),
    })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    dialogVisible.value = false
    await loadFaqs()
  } catch (e) {
    console.error('保存FAQ失败', e)
  }
}

const remove = async (id) => {
  try {
    const res = await fetch(`${API}/faqs/${id}`, { method: 'DELETE' })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    faqs.value = faqs.value.filter(item => item.id !== id)
  } catch (e) {
    console.error('删除FAQ失败', e)
  }
}

onMounted(loadFaqs)
</script>


<style scoped>
.faq {
  margin-left: 220px;
  padding: 24px;
}
.header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 16px;
}
h1 { font-size: 22px; margin: 0; }
button {
  background: #409EFF;
  color: #fff;
  border: none;
  border-radius: 4px;
  padding: 6px 12px;
  cursor: pointer;
  font-size: 13px;
  margin-right: 6px;
}
button.danger { background: #f56c6c; }
.data-table {
  width: 100%;
  border-collapse: collapse;
  font-size: 14px;
  background: #fff;
  border-radius: 8px;
  overflow: hidden;
  box-shadow: 0 1px 4px rgba(0,0,0,0.08);
}
.data-table th {
  text-align: left;
  padding: 10px 12px;
  background: #f5f7fa;
  color: #606266;
  border-bottom: 1px solid #ebeef5;
}
.data-table td {
  padding: 10px 12px;
  border-bottom: 1px solid #ebeef5;
}
</style>