...
export function ProductTranslations({ source }: { source: Source }) {
  ...
  return (
    <section className="sm:col-span-2 lg:col-span-3">
      <h3 className="text-lg font-bold">{t("Product translations")}</h3>
        ...
        >
          ...
          <label>
            ...
            >
              ...
              <option value="READY">{t("Ready for publication")}</option>
              <option value="PUBLISHED">
                {t("Publish complete translation")}
              </option>
            </select>
          </label>
          {(
          ...
          {specifications.map(([key, value]) => (
            <div key={key} className="rounded-lg border p-3">
              ...
              <label>
                ...
                <input
                  className={input}
                  value={translation.specifications?.[key]?.label ?? ""}
                  onChange={(event) =>
                    update(locale, {
                      specifications: {
                        ...
                        [key]: {
                          ...
                          value: translation.specifications?.[key]?.value ?? "",
                        },
                      },
                    })
                  }
              ...
              <label>
                ...
                <input
                  className={input}
                  value={String(translation.specifications?.[key]?.value ?? "")}
                  onChange={(event) =>
                    update(locale, {
                      specifications: {
                        ...translation.specifications,
                        [key]: {
                          label: translation.specifications?.[key]?.label ?? "",
                          value:
                            typeof value === "number"
            ...
            <div key={index} className="rounded-lg border p-3">
              ...
              {(["question", "answer"] as const).map((key) => (
                <label key={key}>
                  ...
                  <textarea
                    className={input}
                    value={translation.productFaq?.[index]?.[key] ?? ""}
                    onChange={(event) =>
                      update(locale, {
                        productFaq: source.productFaq.map((_, i) => ({
                          ...{ question: "", answer: "" },
                          ...translation.productFaq?.[i],
