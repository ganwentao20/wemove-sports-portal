 ...
 */
describe.skipIf(process.env.E2E_DB !== '1' || process.env.E2E_MAIL !== '1')(
  'Mailpit registration and password recovery (SMTP)',
  { timeout: 15000 },
  () => {
    const prisma = new PrismaClient();
    const emails: string[] = [];
    const outboxIds: string[] = [];
    async function flushOwnMail() {
      ...
      for (const row of rows)
        await app.get(NotificationsService).deliver(row.id);
    }
    ...
    async function mailApi(path: string, init?: RequestInit) {
      const response = await fetch(`${inboxUrl}/api/v1${path}`, {
        ...init,
        signal: AbortSignal.timeout(5000),
      });
      if (!response.ok)
        throw new Error(
          `Mailpit ${init?.method ?? 'GET'} ${path.split('?')[0]} failed: ${response.status}`,
        );
      return response;
    }
    async function inbox(email: string): Promise<Inbox> {
      return (
        await mailApi(`/search?query=${encodeURIComponent(`to:${email}`)}`)
      ).json();
    }
    async function message(
      ...
      await expect
        .poll(
          async () => {
            await flushOwnMail();
            id = (await inbox(email)).messages.find(
              (item) => item.Subject === subject && !exclude.includes(item.ID),
            )?.ID;
            return Boolean(id);
          },
    ...
    beforeAll(async () => {
      ...
      const notifications = app.get(NotificationsService),
        enqueue = notifications.enqueue.bind(notifications);
      vi.spyOn(notifications, 'enqueue').mockImplementation(
        async (...input) => {
    ...
    afterAll(async () => {
      try {
        ...
        const ids = [
          ...new Set(
            (await Promise.all(emails.map(inbox))).flatMap((result) =>
              result.messages.map((item) => item.ID),
            ),
          ),
        ];
        if (ids.length)
          await mailApi('/messages', {
            method: 'DELETE',
    ...
    it('registers PENDING, receives a real verification link, activates and logs in', async () => {
      ...
      expect(saved?.userId === user.id && saved?.tokenHash !== token).toBe(
        true,
      );
      expect(
      ...
      const login = await post('login', { email: user.email, password }).expect(
        200,
      );
    ...
    it('receives a reset email, changes the password and rejects reuse', async () => {
      ...
      await post('reset-password', { token, password: newPassword }).expect(
        200,
      );
      await post('reset-password', {
        token,
        password: 'UnexpectedChange123!',
      }).expect(400);
      await post('login', { email: user.email, password }).expect(401);
      await post('login', { email: user.email, password: newPassword }).expect(
        200,
      );
    });
