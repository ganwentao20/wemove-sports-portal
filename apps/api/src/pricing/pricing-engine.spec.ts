...
function rule(
  partial: Partial<PricingRuleCandidate> & {
    id: string;
    scope: PricingRuleCandidate['scope'];
  },
...
describe('pricing-engine · 阶梯档（minQty）', () => {
  it('数量满足时取最高档（更低价）', () => {
    ...
    expect(
      resolveDealerPrice(rules, { ...ctx, quantity: 10 })?.priceCents,
    ).toBe(2200);
    expect(
      resolveDealerPrice(rules, { ...ctx, quantity: 25 })?.priceCents,
    ).toBe(1900);
  });
...
describe('pricing-engine · 零售价与越权边界', () => {
  it('Sale 优先于 MSRP', () => {
    expect(
      resolveRetailPrice({ msrpCents: 3999, salePriceCents: 2999 }),
    ).toEqual({
      priceCents: 2999,
      source: 'SALE',
    });
  ...
  it('无售价时回退 MSRP，两者皆空为 null', () => {
    expect(
      resolveRetailPrice({ msrpCents: 3999, salePriceCents: null })?.source,
    ).toBe('MSRP');
    expect(
      resolveRetailPrice({ msrpCents: null, salePriceCents: null }),
    ).toBeNull();
  });
  ...
  it('游客与普通零售用户不可查看 dealer 价', () => {
    ...
    expect(
      isAllowedDealerPriceView({ kind: 'customer', companyId: null }),
    ).toBe(false);
  });

  it('经销商成员（有企业）与员工可查看', () => {
    expect(isAllowedDealerPriceView({ kind: 'customer', companyId })).toBe(
      true,
    );
