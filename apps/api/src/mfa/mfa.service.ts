 ...
 * - 密钥 base32；允许当前及前一个 30 秒时间步，容纳请求跨步传输，不接受未来时间步。
...
export class MfaService {
  ...
  createSetup(accountEmail: string): { secret: string; otpauthUrl: string } {
    ...
    return {
      secret,
      otpauthUrl: generateURI({ issuer: ISSUER, label: accountEmail, secret }),
    };
  }
  ...
  async verifyWithLimit(
    ...
    if (previousFailures === null && process.env.NODE_ENV === 'production')
      throw new BizException(
        ERROR_CODES.INTERNAL,
        'MFA rate limiter unavailable; try again shortly',
        503,
      );
