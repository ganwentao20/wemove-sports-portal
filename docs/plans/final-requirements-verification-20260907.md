...
商品、零售、内容及相邻权限/售后证据于 2026-09-08 更新；下列局部结果仍不代表最终统一 SHA 的 CI 或全部浏览器验收。
...
| 工作量口径 | 甘文韬 25%；陈婧琳、周慧莹、倪依玲、龙祖怡各 18.75%。这是计划分工，不冒充已核定实际贡献。 |
...
| C03 | §5.2/8.3/9.4：结算、支付与补偿 | 最终重新取价，地址/税运计算证据快照；DEMO/HTTP 适配、HMAC+时间窗+金额/币种验证、事件幂等、失败再试/取消/迟到成功补偿、退款。 | 实现+回归：`retail-commerce`；商户真实协议映射、密钥与实际支付/退款/PCI 范围确认属于外部验收。 |
