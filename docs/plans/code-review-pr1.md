...
- P0 越权问题已由组长侧直接修复并推送分支：commit `cebcd1d`（含迁移
  `20260905070129_dealer_application_applicant` 与回归单测，dealer 单测 6→9 项全绿）。
- 修复内容：归属改为 `applicantId`（登录提交绑定，`OptionalJwtAuthGuard` 可选登录）或
  `companyId`；**邮箱字符串比对已删除**；登录用户重复在途申请 → 409。
- 本地复测通过：同邮箱注册攻击者读取 → 403；提交人本人读取 → 200；陌生人 → 403。
- 待办：组长在 GitHub 合并 PR#1（CI 绿后），P1 附件 mediaId 化建议随 D 的媒体模块落地。
