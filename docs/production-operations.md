...
上述本地运行使用 `finalverify-20260908` 的上轮业务镜像，早于 01:25 完成的零售退款供应商身份/金额/币种校验修订。该业务修订已由主任务单独完成 API 构建及零售数据库回归；包含全部最新业务改动的生产镜像与运行证据，以随后最终提交的 CI 全量重建结果为准，不能将本地旧镜像演练当作该最终镜像已通过。
