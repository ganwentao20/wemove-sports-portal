...
版本：v0.9。日期：2026-09-08。甘文韬组织全项目整合复核，龙祖怡的阶段测试材料另行保留。本报告记录历史整合测试、本次原站方向修正后的工作区回归、本机账号检查和负载实测；每项结论只对应其标明的代码与环境。

## 0 本次原站方向修正后的工作区复核

本次工作区保留 `www.wemovetoy.com` 的 WEMOVE 标识、原木留白风格、首页交错图文结构、栏目与原站图片，并把现有 CMS、导航、商品、多语言、账户、交易和经销商能力接回该前端。修复内容包括：首页模块停用后不再由前端自动补回，通用 CMS 内容块可与原站模块共存；页头与页脚读取后台导航；英文首页和商品主数据不再混入中文；7 个原站产品补充结构化规格和正确适龄下限；增加 `/dealers/apply`、`/play`、`/play/{slug}` 兼容入口及 `/products/{category}` 分类解析。

| 检查 | 本次结果 |
| --- | --- |
| TypeScript | `npm run typecheck` 通过，API 与 Web 均无类型错误 |
| Lint | `npm run lint` 通过 |
| 单元测试 | `npm test` 通过，29 个测试文件、154 个用例全部通过 |
| 数据 | `npm run db:seed` 通过；中文与英文商品名称分别返回正确语言；首页公开接口返回 7 个原站模块 |
| 生产构建 | `npm run build` 通过；Next.js 生成 53 个页面，NestJS 构建通过 |
| 页面人工检查 | `http://localhost:3000/zh?market=CN` 与 `/en?market=US` 均显示原站首页图文结构和原站图片，主要 CTA 保留语言和市场参数 |

本节是当前工作区的本机结果，不冒充新的远端 CI。后文 CI #54、CI #51 和性能数据继续作为对应历史提交的归档证据。
...
| 测试角色 | 用户名 | 密码 | 登录入口 |
| --- | --- | --- | --- |
| 消费者 | customer@wemove.local | Demo1234 | /login |
| 经销商（BUYER） | dealer@wemove.local | Demo1234 | /login |
| 超级管理员 | admin@wemove.local | Demo1234 | /login |

三类测试账号统一使用 `http://localhost:3000/login`（HTTPS：`https://127.0.0.1:3443/login`）；原三入口仅兼容跳转，按身份进入账户中心、企业目录或后台。经销商按要求完成条款或 MFA；员工须绑定验证器并验证当前六位动态码。凭据仅用于测试，重新执行 seed 会把三个演示账号重置为下表密码，禁止对生产库执行演示 seed。
...
测试用户名、密码见第 2 节。项目实际工作量占比为：甘文韬 25%，陈婧琳、周慧莹、倪依玲、龙祖怡各 18.75%，合计 100%。
...
原始历史报告 v0.2/v0.6、CI #49 证据及成员贡献记录保留原版本，不把旧数字移植到新提交。最终冻结源码提交以提交包内 `FINAL-SOURCE-SHA.txt` 为准；是否合入 main 与冻结代码树通过测试是两件不同的事实。
